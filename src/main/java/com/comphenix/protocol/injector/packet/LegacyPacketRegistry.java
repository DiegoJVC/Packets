...
class LegacyPacketRegistry {	
	...
	public void initialize() {
		if (packetToID == null) {
			...
			customIdToPacket = InverseMaps.inverseMultimap(packetToID, new Predicate<Entry<Class, Integer>>() {
				@Override
			...
			vanillaIdToPacket = InverseMaps.inverseMap(packetToID, new Predicate<Entry<Class, Integer>>() {
				@Override
				public boolean apply(@Nullable Entry<Class, Integer> entry) {
					return MinecraftReflection.isMinecraftClass(entry.getKey());
				}
